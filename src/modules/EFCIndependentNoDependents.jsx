...
export default function EFCIndependentNoDependents() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3001/get/efcindnodep")
    .then((response) => {
      if(response.ok) {
        try {
          JSON.parse(response);
  ...
  const handleSubmit = (event) => {
    ...
    fetch('http://localhost:3001/post/efcindnodep', {
      ...
      <div className="row g-0">
        ...
        <div className="col-10 content-area">
          <form onSubmit={handleSubmit}>
            <h3>EFC - Independent with no Dependents</h3> 
              ...
              <tbody>
                {
                  ...
                  <tr className="void-row">
                    <td colSpan="11">{inputList.length - 1 === i && <button className="add-button" onClick={handleAddClick}>Add Row</button>}</td>
                  </tr>
                  </React.Fragment>
                );
